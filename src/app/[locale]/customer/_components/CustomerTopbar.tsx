  ...
  return (
    <header className="sticky top-0 inset-x-0 z-50 bg-white border-b border-neutral-200 shadow-[0_1px_0_0_rgba(0,0,0,0.03)]">
      <div className="mx-auto max-w-6xl px-4 md:px-6 h-16 flex items-center justify-between gap-4">
        {/* Brand (left) */}
        <Link href={{ pathname: "/customer" }} className="flex items-center gap-3 min-w-0">
          ...
          <span className="text-lg font-black tracking-tight truncate text-neutral-900">{t("appTitle")}</span>
        </Link>

        {/* Nav + actions (right) */}
        <div className="flex items-center gap-3">
          <nav className="hidden md:flex items-center gap-6 text-sm font-semibold text-neutral-700">
            <Link href={{ pathname: "/customer" }} className="text-emerald-600">Menu</Link>
            <Link href={{ pathname: "/about" }} className="hover:text-emerald-600 transition-colors">About</Link>
            <Link href={{ pathname: "/contact" }} className="hover:text-emerald-600 transition-colors">Contact</Link>
          </nav>
          <LanguageSwitcher />
          <CartSheet>
