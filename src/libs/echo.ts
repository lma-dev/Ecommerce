...
export function getEcho(): Echo | null {
  ...
  if (!window.__app_echo) {
    ...
    try {
      const log = String(process.env.NEXT_PUBLIC_PUSHER_LOG || "").toLowerCase();
      if (log === "true" || log === "1" || log === "yes") {
        (Pusher as any).logToConsole = true;
      }
    } catch {}
    const useTokenAuth = String(process.env.NEXT_PUBLIC_ECHO_TOKEN_AUTH || "").toLowerCase() === "true";
    ...
    if (useTokenAuth && backendUrl) {
      baseConfig.authorizer = (channel: any, options: any) => {
        return {
    ...
    window.__app_echo = new Echo(baseConfig) as unknown as Echo;
  }
