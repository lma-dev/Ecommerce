...
class CustomerResetPassword extends ResetPassword implements ShouldQueue
{
    ...
    {
        $expiration = config('auth.passwords.customers.expire', config('auth.passwords.' . config('auth.defaults.passwords') . '.expire'));

        return (new MailMessage)
            ->subject(__('Reset Your Password'))
            ->line(__('We received a password reset request for your customer account.'))
